...
receiver-action checks through retreat, and live timing/freshness admission
before one-time commitment. Its contribution is this auditable integration.
The comparison below positions that formulation; it does not establish
priority over all handover methods or empirical superiority.
...
| **TRIAD** | Finite complete-plan enumeration, model-relative hard checks, seven-term ranking, live timing/freshness admission, and governed execution after one commitment | Simulation outcomes and archived controller timing profiles |

Yang's goal-set formulation jointly reasons about grasp choice and motion;
it is not accurately described as simply choosing a grasp and then planning
motion. Its approach uses MPC, while its grasp phase uses a blocking policy.
See the [primary paper, Sections III and V](https://arxiv.org/pdf/2204.00134).
...
TRIAD's reported evaluation does not include matched runs against these
systems. No claim of better completion, speed, physical safety, or human
preference follows from this table. The [method](mathematics.md) and
[results](results.md) define the narrower claims supported here.
