...
\item A matched comparison in which the more elaborate TRIAD-lite mode loses to a plain predictive baseline (Section~\ref{sec:lite}).
...
\textbf{What is left to TRIAD.} The combination is an implementation choice: a finite, inspectable plan set with a certification stage that models closure and retreat before commitment, on a task-space QP executor. Explicitness of the plan is a property of the implementation, not a claim against those methods.
...
\begin{figure}[t]\centering\includegraphics[width=0.98\columnwidth]{F01_architecture_four_layer.pdf}
...
\begin{figure}[t]\centering\includegraphics[width=0.98\columnwidth]{F05_finite_bank.pdf}
...
\begin{figure}[t]\centering\includegraphics[width=0.98\columnwidth]{F06_frozen_live_pipeline.pdf}
\caption{Frozen-state certification versus live timing admission.}\label{fig:pipeline}\end{figure}

\subsection{Receding and supervisory modes}
A receding mode (V2) replaces the single frozen search by a supervisor that re-certifies the active plan, cancels and restarts the search when the object estimate changes, and commits at a terminal boundary; it uses a robot-independent scripted giver. A supervisory mode (TRIAD-lite) adds a sampled earliest-feasible rendezvous solver over a 530-hypothesis grasp family and a directional velocity-authority filter $\kappa$ computed from the QP's joint-velocity box. All modes share the acquisition interface: closure is gated on the object being at rest (4 mm/s).

\section{What the Instrumentation Showed}\label{sec:limits}
\textbf{The event time is imposed in the V1 giver model.} In the V1 configuration the simulated giver follows the committed presentation time after commitment. The event time is therefore not a prediction that the world may or may not honour; it is a schedule the robot sets. The V2 independent giver removes this, at the cost of the moving-epoch search (2.9 to 3.9 s) exceeding the constant-velocity window (3.67 s).
...
\begin{figure}[t]\centering\includegraphics[width=0.98\columnwidth]{F13_feasibility_funnel.pdf}
...
\subsection{Matched comparison of the TRIAD-lite mode}\label{sec:lite}
...
\begin{figure}[t]\centering\includegraphics[width=0.98\columnwidth]{two_robot_sim.png}
...
The controller, the configuration, the evidence records, the two-robot setup and the scripts that regenerate every number in this paper are in the TRIAD repository (tag \texttt{triad-2026-09-24}). The unit tests and documentation-claim guards run in continuous integration; the two-arm simulation runs from a build tree with one script.

\section{Conclusion}\label{sec:conc}
TRIAD is an auditable finite-plan receiver whose decision structure is prior art and whose own instrumentation shows that timing dominates selection, that the V1 giver model imposes the event time, and that the more elaborate TRIAD-lite mode loses to a plain predictive baseline. Its value is the implementation, the attribution, the measured limits, and a robot-to-robot setup on which the next question can be tested on hardware.
