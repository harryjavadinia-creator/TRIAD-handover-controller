...
run char_on v2 near-ground char_on
for s in $SC; do run v1 v1 $s ""; done
for s in $SC; do run char_off v2 $s char_off; run char_on v2 $s char_on; done
for rep in a b c; do for s in $SC; do run off_$rep v2 $s ""; run on_$rep v2 $s prune_on; done; done
echo CAMPAIGN_DONE
