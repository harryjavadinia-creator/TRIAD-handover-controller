#include "HandoverInterceptionController_CaptureTransfer.h"
...
void HandoverInterceptionController_CaptureTransfer::configure(
...
void HandoverInterceptionController_CaptureTransfer::start(
    mc_control::fsm::Controller & ctl_)
{
  ...
  loadTransferElapsed_ = 0.0;
  releaseStableTime_ = 0.0;
  admittanceOffset_ = 0.0;
  admittanceVelocity_ = 0.0;
  acquireTimingFinished_ = false;
  ...
  transferTarget_ = centeredTarget_;
  ...
  {
    mc_rtc::log::error(
        "[ForceAwareCaptureTransfer] cannot close: {}. Load the actuated kinova_gen3_2f85 RobotModule",
        ctl.gripperActuationStatus());
  }

  mc_rtc::log::warning(
      "[ForceAwareCaptureTransfer] V6.2 source={} oneContinuousController=true virtualLoadSharing=true blend={:.2f}s centerTube={:.4f}->{:.4f}m predictedContact={:.3f}",
      ctl.forceTransferSourceDescription(), priorityBlendDuration_,
      ctl.acquisitionFarCenterTolerance(),
      ctl.acquisitionNearCenterTolerance(),
      ctl.selectedCandidateContactClosure());
}

bool HandoverInterceptionController_CaptureTransfer::run(
    mc_control::fsm::Controller & ctl_)
{
  auto & ctl = static_cast<HandoverInterceptionController &>(ctl_);
  ++iter_;
  ...
  {
    mc_rtc::log::error("[ForceAwareCaptureTransfer] certified candidate or actuated gripper unavailable");
    output("FAIL");
    return true;
  }

  if(phase_ != Phase::LoadTransfer
     && ctl.committedPlanValid()
  ...
  const bool intentionalPadContactMode =
      phase_ == Phase::Confirming || phase_ == Phase::LoadTransfer;
  bool closureSafe = ctl.evaluateCurrentClosureSafety(
      report, false, intentionalPadContactMode);

  // Hardware contact can cross the strict pre-contact numerical penetration
  // tolerance by one or two samples before the Closing phase has accumulated
  // its bilateral-contact dwell. Re-evaluate only the two designated
  // inner-pad/blue-handle pairs with the controller's existing contact band.
  // Every other gripper/object/arm/environment constraint remains hard.
  if(!closureSafe && phase_ == Phase::Closing)
  {
    HandoverSafetyReport contactReport;
    const bool designatedContactSafe =
        ctl.evaluateCurrentClosureSafety(contactReport, false, true);
    if(designatedContactSafe && contactReport.bilateralPadContact)
    {
      ...
      {
        mc_rtc::log::warning(
            "[AcquireContactEntry] bounded bilateral hardware contact admitted during Closing using the existing contact band left={:.4f} right={:.4f} centerErr={:.4f}; all non-contact safety constraints remain hard",
  ...
  if(phase_ == Phase::LoadTransfer)
  {
    loadTransferElapsed_ += ctl.controlDt();
    const auto & plan = ctl.committedInterceptionPlan();
    ctl.setVirtualForceTransferState(
        admittanceOffset_, admittanceVelocity_, bilateralContactInsideTube);
    const auto & measurement = ctl.forceTransferMeasurement();

    // Preserve the exact selected mouth target and centering servo. The only
    // added degree of freedom is a bounded displacement along the committed
    // object's load axis, generated by the force-admittance equation.
    const double desiredShare = smoothStep01(
        loadTransferElapsed_ / std::max(1e-9, plan.forceTransferDuration
                                                   - plan.forceTransferReleaseDwell));
    const double desiredLoad = desiredShare * plan.forceTransferFullSupportForce;
    const double loadError = desiredLoad - measurement.loadForce;
    const double acceleration = (
        loadError
        - plan.forceTransferAdmittanceDamping * admittanceVelocity_
        - plan.forceTransferAdmittanceStiffness * admittanceOffset_)
        / std::max(1e-9, plan.forceTransferAdmittanceMass);
    admittanceVelocity_ = clampValue(
        admittanceVelocity_ + acceleration * ctl.controlDt(),
        -plan.forceTransferMaximumAdmittanceSpeed,
        plan.forceTransferMaximumAdmittanceSpeed);
    admittanceOffset_ = clampValue(
        admittanceOffset_ + admittanceVelocity_ * ctl.controlDt(),
    ...
    transferTarget_ = centeredTarget_;
    transferTarget_.translation() +=
        measurement.loadAxisWorld * admittanceOffset_;
    ctl.commandMouthTarget(transferTarget_);
    ...
    {
      mc_rtc::log::error(
          "[ForceTransferGate] invalid transfer signal source={}; refusing attachment and retreat",
          ctl.forceTransferSourceDescription());
      output("FAIL");
      return true;
    }

    if(measurement.forceWorld.norm() > plan.forceTransferMaximumForce
       || measurement.momentNorm > plan.forceTransferMaximumMoment)
    {
      mc_rtc::log::error(
          "[ForceTransferGate] hard wrench bound exceeded force={:.3f}/{:.3f}N moment={:.3f}/{:.3f}Nm; fail-safe hold",
          measurement.forceWorld.norm(), plan.forceTransferMaximumForce,
          measurement.momentNorm, plan.forceTransferMaximumMoment);
      output("FAIL");
      return true;
    }

    const bool releaseEvidence =
        bilateralContactInsideTube
        && measurement.transferIndex >= plan.forceTransferReleaseThreshold
        && std::abs(measurement.transferRate)
           <= plan.forceTransferReleaseRateTolerance;
    if(releaseEvidence) { releaseStableTime_ += ctl.controlDt(); }
    else { releaseStableTime_ = 0.0; }

    if(iter_ % 50 == 1)
    {
      mc_rtc::log::info(
          "[ForceTransferGate] t={:.3f}/{:.3f}s source={} synthetic={} simulatedPhysics={} desiredShare={:.3f} measuredShare={:.3f} rate={:+.3f}/s robotLoad={:.3f}/{:.3f}N humanSupport={:.3f}N lateral={:.3f}N moment={:.3f}Nm virtualDeflection={:+.4f}m admittance=[{:+.4f}m,{:+.4f}m/s] bilateral={} releaseStable={:.3f}/{:.3f}s",
          loadTransferElapsed_, plan.forceTransferTimeout,
          ctl.forceTransferSourceDescription(), measurement.synthetic,
          measurement.simulatedPhysics, desiredShare,
          measurement.transferIndex, measurement.transferRate,
          measurement.loadForce, plan.forceTransferFullSupportForce,
          measurement.humanSupportForce, measurement.lateralForce,
          measurement.momentNorm, measurement.virtualContactDeflection,
          admittanceOffset_, admittanceVelocity_, bilateralContactInsideTube,
          releaseStableTime_, plan.forceTransferReleaseDwell);
    }

    if(releaseStableTime_ >= plan.forceTransferReleaseDwell)
    {
      ctl.commitAcquiredMouthTarget(transferTarget_);
      if(!ctl.attachObjectToMouth())
      {
        mc_rtc::log::error(
            "[ForceTransferComplete] attachment refused despite stable bilateral grasp and transfer evidence");
        output("FAIL");
        return true;
      }
      ctl.finishForceTransferExecution();
      ctl.finishPhaseTiming("transfer");
      mc_rtc::log::success(
          "[ForceTransferComplete] robot support established and release evidence stable share={:.3f} robotLoad={:.3f}N humanSupport={:.3f}N rate={:+.3f}/s source={} synthetic={} simulatedPhysics={}; certified retreat admitted",
          measurement.transferIndex, measurement.loadForce,
          measurement.humanSupportForce, measurement.transferRate,
          ctl.forceTransferSourceDescription(), measurement.synthetic,
          measurement.simulatedPhysics);
      output("OK");
      return true;
    }

    if(loadTransferElapsed_ >= plan.forceTransferTimeout)
    {
      mc_rtc::log::error(
          "[ForceTransferGate] load takeover/release evidence did not stabilize before timeout share={:.3f} rate={:+.3f}/s bilateral={} source={}",
          measurement.transferIndex, measurement.transferRate,
          bilateralContactInsideTube,
          ctl.forceTransferSourceDescription());
      output("FAIL");
  ...
  if(phase_ == Phase::Confirming)
  {
    ...
    if(confirmationStableTime_ >= confirmationDwell_)
    {
      ...
      closeCommand_ = holdClosure_;
      transferTarget_ = centeredTarget_;
      phase_ = Phase::LoadTransfer;
      loadTransferElapsed_ = 0.0;
      releaseStableTime_ = 0.0;
      admittanceOffset_ = 0.0;
      admittanceVelocity_ = 0.0;
      ctl.finishPhaseTiming("acquire");
      acquireTimingFinished_ = true;
      ctl.startPhaseTiming("transfer");
      ctl.beginForceTransferExecution();
      ctl.setVirtualForceTransferState(0.0, 0.0, true);
      mc_rtc::log::success(
          "[CaptureTransferBoundary] bilateral grasp confirmed; same target/gains/posture/centering/contact regulator continue into force-aware load transfer closure={:.3f} left={:.4f} right={:.4f} centerErr={:.4f}",
...
void HandoverInterceptionController_CaptureTransfer::teardown(
    mc_control::fsm::Controller & ctl_)
{
  auto & ctl = static_cast<HandoverInterceptionController &>(ctl_);
  if(!acquireTimingFinished_) { ctl.finishPhaseTiming("acquire"); }
  if(phase_ == Phase::LoadTransfer)
  {
    ctl.setVirtualForceTransferState(0.0, 0.0, false);
    ctl.finishForceTransferExecution();
    ctl.finishPhaseTiming("transfer");
  }
  mc_rtc::log::info("[ForceAwareCaptureTransfer] teardown");
}

EXPORT_SINGLE_STATE("HandoverInterceptionController_CaptureTransfer",
